...
if __name__ == "__main__":
    ...
    build_yolo_cls_dataset_from_subfolders(
        src_root_dir="Body_Frame",
        out_dataset_dir="Bike_BODY_output"
    )

    build_yolo_cls_dataset_from_subfolders(
        src_root_dir="Braking & Suspension",
        out_dataset_dir="Bike_BRAKING_output"
    )

    build_yolo_cls_dataset_from_subfolders(
        src_root_dir="Cooling & Lubrication",
        out_dataset_dir="Bike_cooling_output"
    )

    build_yolo_cls_dataset_from_subfolders(
        src_root_dir="Electrical and Ignition",
        out_dataset_dir="Bike_ELECTRICAL_output"
    )

    build_yolo_cls_dataset_from_subfolders(
        src_root_dir="Engine & Core Mechanical",
        out_dataset_dir="Bike_ENGINE_output"
    )

    build_yolo_cls_dataset_from_subfolders(
        src_root_dir="Fuel & Air Intake",
        out_dataset_dir="Bike_FUEL_output"
    )

    build_yolo_cls_dataset_from_subfolders(
        src_root_dir="Transmission & Drivetrain",
        out_dataset_dir="Bike_TRANSMISSION_output"

    )
    build_yolo_cls_dataset_from_subfolders(
        src_root_dir="Lighting & Signal",
        out_dataset_dir="Bike_LIGHTS_output"
    )